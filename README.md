...
- 「全カードに適用」で、いま編集中のカードの見た目（フォント・色・テンプレートなど）を他のカードにも一括で反映（商品名・価格・画像は変わりません）

### 出力・保存
- **PNG保存**（150 / 300 / 600 dpi）— 300dpi なら印刷しても文字がにじみません。「シート（面付け済み）」「選択中のカードのみ」を選べ、シートは全ページ一括保存にも対応
- **印刷** — 面付けされたシートを、用紙サイズに合わせた原寸で1回の印刷ダイアログにまとめて出します
...
assets/js/
  ...
  preset-ui.js          保存したデザインの一覧と操作
  export-tool.js        出力（面付けシートの印刷・PNG保存）
...
- 文字設定のタブは ARIA タブパターン対応（`←`/`→`/`Home`/`End` で移動）
